...
public class OpenWarRight : MonoBehaviour
{
    ...
    public void Update()
    {
        ...
        {
            t += Time.deltaTime;
            WardrobeHolderRight.transform.rotation = Quaternion.Lerp(closeGoal.rotation, openGoal.rotation, t);
            if (t >= 1)
            {
        ...
        {
            t += Time.deltaTime;
            WardrobeHolderRight.transform.rotation = Quaternion.Lerp(openGoal.rotation, closeGoal.rotation, t);
            if (t >= 1)
            {
                currentState = state.None;
                t = 0;
